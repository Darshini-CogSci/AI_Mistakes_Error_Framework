"""
t-SNE of layer2 embeddings for images labelled knife (true/false)
...
LAYER2_FILES = {
    'silhouette':   "./path/to/your/embedding/file/", # a file containing silhouette condition embeddings for layer2 layer
...
print('Loading trial data and layer2 embeddings...')
knife_rows = []

for condition, csv_path in TRIAL_CSVS.items():
    with open(LAYER2_FILES[condition]) as f:
        layer2_emb = {k: np.array(v) for k, v in json.load(f).items()}
...
print(f'\nlayer2 dim: {X.shape[1]}, vectors: {len(X)}')
print('Running PCA...')
...
ax.legend(handles=label_handles, bbox_to_anchor=(1.01, 0.45),
          loc='upper left', fontsize=9, title='Label type')

ax.set_title('layer2 embeddings: knife true/false labels across conditions\n'
             'Circles = true knife | X (black edge) = false knife', fontsize=11)
...
plt.savefig('results/plots/layer2_knife_tsne.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved -> results/plots/layer2_knife_tsne.png')
