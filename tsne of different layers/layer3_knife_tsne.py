"""
t-SNE of layer3 embeddings for images labelled knife (true/false)
...
LAYER3_FILES = {
    'silhouette':   "./path/to/your/embedding/file/", # a file containing silhouette condition embeddings for layer3 layer
...
print('Loading trial data and layer3 embeddings...')
knife_rows = []

for condition, csv_path in TRIAL_CSVS.items():
    with open(LAYER3_FILES[condition]) as f:
        layer3_emb = {k: np.array(v) for k, v in json.load(f).items()}
...
print(f'\nlayer3 dim: {X.shape[1]}, vectors: {len(X)}')
print('Running PCA...')
...
ax.set_title('layer3 embeddings: knife true/false labels across conditions\n'
...
plt.savefig('results/plots/layer3_knife_tsne.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved -> results/plots/layer3_knife_tsne.png')
