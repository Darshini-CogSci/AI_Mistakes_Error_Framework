"""
t-SNE of conv1 embeddings for images labelled knife (true/false)
...
CONV1_FILES = {
    'silhouette':   "./path/to/your/embedding/file/", # a file containing silhouette condition embeddings for conv1 layer
...
print('Loading trial data and conv1 embeddings...')
knife_rows = []

for condition, csv_path in TRIAL_CSVS.items():
    with open(CONV1_FILES[condition]) as f:
        conv1_emb = {k: np.array(v) for k, v in json.load(f).items()}
...
print(f'\nConv1 dim: {X.shape[1]}, vectors: {len(X)}')
print('Running PCA...')
...
label_handles = [
    ...
    plt.Line2D([0],[0], marker='o', color='gray', linestyle='none',
               markersize=8,
               label='false knife (o, black edge)'),
]
...
ax.set_title('Conv1 embeddings: knife true/false labels across conditions\n'
...
plt.savefig('results/plots/conv1_knife_tsne.png', dpi=150, bbox_inches='tight')
plt.show()
print('Saved -> results/plots/conv1_knife_tsne.png')
